        ...
        if 'block' in phedexOut['phedex']:
            ...
            if 'file' in phedexOut['phedex']['block'][0]:
                ...
                if phedexOut['phedex']['block'][0]['file'][0]['name'] == fileName:
                    sites = []
                    noTapesites = []
                    for siteDict in phedexOut['phedex']['block'][0]['file'][0]['replica']:
                        if siteDict['node'] not in sites:
                            sites.append(siteDict['node'])
                        notAppend = False
                        if siteDict['node'].endswith('MSS') or siteDict['node'].endswith('Export') or siteDict['node'].endswith('Buffer'):
                            notAppend = True
                        if not notAppend:
                            noTapesites.append(siteDict['node'])
                    return {'checksum': phedexOut['phedex']['block'][0]['file'][0]['checksum'],
                            'bytes': phedexOut['phedex']['block'][0]['file'][0]['bytes'],
                            'sites': sites, 'withoutTapeSites': noTapesites}
    return {}
...
def execute(dirPath):
    ...
    for root, dirs, files in os.walk(dirPath):
         for fName in files:
             fullLocation = "%s/%s" % (root, fName)
#             fileLoc = getFileLocations(corruptFile)
             fileInfo = getFileInfoFromPhedex(fullLocation[11:])
             print '%s %s %s %s' % (fullLocation, fileInfo['bytes'], len(fileInfo['withoutTapeSites']), len(fileInfo['sites']))
...
if __name__ == "__main__":
    execute('/mnt/hadoop/store/data/')
    execute('/mnt/hadoop/store/mc/')
